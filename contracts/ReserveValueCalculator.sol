...
contract StrikeReserveValueCalculator is Ownable {
    ...
    function updateReserveTokenDetails(
        ...
        bool _isStable // If the reserve token is a stable coin
    ...
    /**
        @notice gets the market value of a reserve token in 100ths so 1 MIM is 100
        @param _reserve address
        @return _value uint
     */
    function valueOfReserveToken( address _reserve ) public view returns ( uint _value ) {
        if (reserveDetails[ _reserve ].isStable == true){
            ...
            uint256 pairStableBalance = ( IERC20( reserveDetails[ _reserve ].stable ).balanceOf( reserveDetails[ _reserve ].lp ).mul( reserveDetails[ _reserve ].controlVariable ).mul( 10 ) ) / ( 10 ** IERC20( reserveDetails[ _reserve ].stable ).decimals() );
            uint256 pairPrincipleBalance = ( IERC20( reserveDetails[ _reserve ].principle ).balanceOf( reserveDetails[ _reserve ].lp ) ) / ( 10 ** IERC20( reserveDetails[ _reserve ].principle ).decimals() );
